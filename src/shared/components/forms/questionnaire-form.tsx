...
export function QuestionnaireForm({ mode, initialData, onSubmit, onCancel }: QuestionnaireFormProps) {
  ...
  function validate(): boolean {
    ...
    if (!form.title.trim()) e.title = 'כותרת השאלון נדרשת';
  ...
  const set = <K extends keyof QuestionnaireFormData>(k: K, v: QuestionnaireFormData[K]) =>
  ...
  return (
    <div style={{ direction: 'rtl' }}>
      ...
      <div style={fieldStyle}>
        ...
        <input style={inputStyle} value={form.title} onChange={e => set('title', e.target.value)} placeholder="שם השאלון" />
      ...
      <div style={fieldStyle}>
        ...
        <textarea style={{ ...inputStyle, minHeight: 70, resize: 'vertical' }} value={form.description} onChange={e => set('description', e.target.value)} placeholder="תיאור השאלון ומטרתו" />
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <input style={inputStyle} value={form.assignedTo} onChange={e => set('assignedTo', e.target.value)} placeholder="שם הממלא (אופציונלי)" />
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
        ...
        >
          {loading ? 'שומר...' : mode === 'create' ? 'הוסף שאלון' : 'עדכן שאלון'}
        </button>
        <button
          onClick={onCancel}
          style={{
